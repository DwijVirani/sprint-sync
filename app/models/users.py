...
class ProfileResponse(BaseModel):
    ...
    id: int
    email: str = Field(
        ..., description="The user's email address", example="user@example.com"
    )
    ...
    isAdmin: bool = Field(
        ..., description="Indicates if the user has admin privileges", example=False
    )
    current_organization_id: Optional[int] = Field(
        None, description="Currently selected organization ID", example=1
    )
    current_organization: Optional[OrganizationResponse] = Field(
        None, description="Currently selected organization details"
    )
    organizations: Optional[List[UserOrganizationResponse]] = Field(
        None, description="All organizations the user belongs to"
    )
