...
from app.db.entities import (Organization, Task, TaskStatus,
                             TaskStatusTransition, TaskWorkflowTransition)
from app.models.workflow import (OrganizationCreate, TaskStatusCreate,
...
def setup_example_workflow():
    ...
    org_data = OrganizationCreate(
        name="Acme Software", description="Software development company"
    )
    ...
    transitions = [
        # From New
        TaskWorkflowTransitionCreate(
            organization_id=1, from_status_id=1, to_status_id=2  # New  # In Progress
        ),
        # From In Progress
        TaskWorkflowTransitionCreate(
            organization_id=1,
        ...
        # From QA Testing
        TaskWorkflowTransitionCreate(
            organization_id=1, from_status_id=4, to_status_id=5  # QA Testing  # Done
        ),
        TaskWorkflowTransitionCreate(
            organization_id=1,
        ...
        TaskWorkflowTransitionCreate(
            organization_id=1, from_status_id=5, to_status_id=2  # Done  # In Progress
        ),
    ]

    return {"organization": org_data, "statuses": statuses, "transitions": transitions}
...
def example_task_workflow():
    ...
    return {"task": task_data, "transitions": transitions}
...
if __name__ == "__main__":
    ...
    print(
        f"Created workflow with {len(workflow['statuses'])} statuses and {len(workflow['transitions'])} transitions"
    )
    print()
